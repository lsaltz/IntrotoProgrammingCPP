...
Mancala 6
4/27/2020
Description: determines if mancala game is over
*/
#include <iostream>
#include <iomanip>
using namespace std;
...
int main( )
{
	...
	int winner; //winner of game (-1 no winner, 1 player1 wins, 2 player2 wins, 3 tie)
...
int gameOverCheck( int beadArray[MAX] )
{
	...
	for ( int i = 0; i < 6; i++ )
	{
		beadSum1 = beadArray[i];
		player1Sum = beadArray[i] + beadSum1;
	}

	for ( int i = 7; i < 13; i++ )
	{
		beadSum2 = beadArray[i];
		player2Sum = beadArray[i] + beadSum2;
	}
...
//Problems: put functions in wrong order, didn't add or zero correctly